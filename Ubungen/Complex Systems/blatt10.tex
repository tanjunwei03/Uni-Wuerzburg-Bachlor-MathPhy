...
\begin{proof}
	...
	\item The probability is given by
		\[
			\mathbb{P}(\sigma^z=1) = \mel{\psi}{\sigma^z}{\psi}
		\]
		where $\ket{\psi}$ is the vector within the 1-dimensional eigenspace $\sigma^z=1$. In this basis, this eigenvector is given by $\ket{\psi}=(1,0)^T$. Substituting, we find that
		\[
...
\begin{proof}
	...
	\item Note on the indices in $c_{ik}$: $i$ goes from 1 to $N$ and $k$ goes from 1 to $K<N$. Thus, for each $k$, we have a vector in $\C^N$. The $c$s are orthonormal for different $i$. By the basis extension theorem, we can extend this basis to an orthonormal basis of $\C^N$, i.e. pick $c_{ik}$ for $k>K$ such that $c_{ik}$s are orthonormal.
		...
		\[
			\sqrt{p_i}\ket{\psi_i} = \sum_{k=1}^K c_{ik}\sqrt{\lambda_k} \ket{k}
		,\]
		we must extend the kets $\ket{k}$ for $k>K$ by just choosing kets within the kernel of $\rho$; since then $\lambda_k=0$, this makes no difference to the sum.
...
\{\{1/Sqrt[2], - 1/Sqrt[2]\}, \{1/Sqrt[2], 1/Sqrt[2]\}\}
		\end{mmaCell}
		In a better formatted manner, we have the coefficients $c_{ik}$ form the matrix
		\[\mathbf{c} = \frac1{\sqrt{2}}\begin{pmatrix}
			1 & -1 \\ 1 & 1
		\end{pmatrix}\]
		Note that this is not the same basis that the vectors are given in! The $c_{ik}$s should be interpreted merely as numbers stored easily in this $2\times 2$ array. 
		
The $\mathbf{T}$ matrix then follows easily, from $\mathbf{T} = |\mathbf{c}|^2$ elementwise as
\[
	\mathbf T = \frac{1}{2}\begin{pmatrix} 1 & 1 \\ 1 & 1 \end{pmatrix} 
.\] 
It is obvious that $\mathbf T$ has all real and nonegative entries. It is also obvious that all rows and columns sum up to 1. 

Finally, we can compute the product
\[
	\mathbf T\vec{\lambda} = \frac{1}{2}\begin{pmatrix}1 & 1 \\ 1 & 1  \end{pmatrix} \begin{pmatrix} (2+\sqrt{2}) / 4 \\ (2 - \sqrt{2} ) / 4   \end{pmatrix} = \begin{pmatrix} 1 / 2 \\ 1 / 2 \end{pmatrix} =\vec{p} 
.\qedhere\] 
	\end{parts}
\end{proof}
