...
\item The Shannon entropy is given by
	\[
		H(X) = \mathbb{E}[-\log f(X)]=-\sum_x f(x)\log f(x)
	.\] 
...
\item The binomial coefficient is given by
	...
	.\] 
\item To compute the entropy, we approximate the sum $-\sum P_k \log_2 P_k$ with an integral  $-\int P_k(x)\log_2 P_k(x) \dd{x}$. Since the exponential falls very fast, we can assume that the integration bounds extend to infinity on both sides. Additionally, for brevity, instead of computing this for $\log_2 P_k$, we will instead use $\ln P_k = \log_2 P_k / \log_2 e$. We will multiply this in the end to get the desired answer.
	\[
	...
	.\] 
	This is a simple gaussian integral, which can be evaluted easily by hand. We do it using Mathematica to find
	\begin{mmaCell}{Input}
Integrate[Sqrt[2/(Pi n)] Exp[-2 (k - n/2)^2 / n] (1/2 Log[2/(Pi n)] - 2 (k-n/2)^2 / n), \{k, -Infinity, Infinity\}]
	\end{mmaCell}
...
1/2 (-1 + Log[2/(n Pi)])
	...
	or, better formatted,
	\begin{align*}
		S &= -\frac{1}{2}\left[ -1 + \ln \frac{2}{n\pi} \right]  \\
		&= -\frac{1}{2}\left[ \ln \frac{2}{n\pi}-\ln e \right]  \\
		&= \frac{1}{2}\ln \frac{n\pi e}{2} 
	\end{align*}
	To get the expressin in bits, we have to multiply by $\log_2 e$, or divide by $\ln 2$, to get
	\[
	S = \frac{1}{2}\log_2 \frac{n\pi e}{2}
	.\] 
	The entropy grows with $\log_2 n$. 
		\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
				width=12cm,
				...
				\addlegendentry{$\frac{1}{2}\log_2 \frac{n\pi e}{2}$}
				
								% Second line
			\end{axis}
		\end{tikzpicture}
	\end{center}
	As can be seen in the figure above, the expression converges to the exact entropy as $n\to\infty$, as we might expect.\qedhere
\end{parts}
\end{proof}
...
\begin{proof}
	...
	\item The discrete Shannon entropy is given by
		\begin{align*}
			...
			&= -\sum_j \frac{p(j\Delta x)\Delta x}{\mathcal{N}} \ln p(j\Delta x) - \sum_j \frac{p(j\Delta x)\Delta x}{\mathcal{N}}\ln \frac{\Delta x}{\mathcal{N}} \\
			&= -\sum_j \frac{p(j\Delta x)\Delta x}{\mathcal{N}} \ln p(j\Delta x) - \ln \frac{\Delta x}{\mathcal{N}} 
		\end{align*}
		In the limit of $\Delta x\to 0$, we expect sums to approach a Riemann integral in the normal (net convergence) sense. As a first consequence, this means that
		\[
			\mathcal{N}=\sum_{j=-\infty}^{\infty} p(j\Delta x)\Delta x \xrightarrow{\Delta x \to 0}\int_{-\infty}^\infty p(x)\dd{x} = 1 
		.\] 
		We will thus have
		\[
			-\sum_j \frac{p(j\Delta x)\Delta x}{\mathcal{N}}\ln p(j\Delta x)\xrightarrow{\Delta x \to 0}-\int_{-\infty}^\infty p(x)\ln p(x)\dd{x}=H_c 
		.\] 
		This means that
		\[
			H\xrightarrow{\Delta x \to 0}H_c - \ln \Delta x
		\] 
		from which we identify $\Delta H = -\ln \Delta x$ as desired.
	\item The leading order expression remains $\Delta H = -\ln \Delta x$. It diverges, as $\Delta x \to 0$.
	\item As the bins become smaller, the number of possible outcomes increase. Thus, we require more information to represent this distribution, resulting in a divergent entropy. In this case, only entropy differences or relative entropies are physical.\qedhere
	\end{parts}
\end{proof}
