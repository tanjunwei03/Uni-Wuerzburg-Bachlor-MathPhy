...
\begin{proof}
	...
	\item The grand canonical partition function is given by
		...
		\end{align*}
		where $N(n_x,n_y,n_z)$ is the number of particles in the state $(n_x,n_y,n_z)$. Notably, $n_x, n_y$ and $n_z$ represent just energy states, rather than the number of bosons in a state. Taking the logarithm yields
		\[
			\ln Z_G = -\sum_{n_x,n_y,n_z=0}^\infty\ln\left[ 1 - ze^{-\beta \hbar\omega(n_x+n_y+n_z)} \right] 
		.\] 
...
.\] 
\item In general, a sum can be approximated by an integral when it does not change much over an interval of length 1. This occurs when the energy spacing between the levels is small, i.e. $\beta\hbar\omega \le 1$.
...
\[
	\langle N\rangle=-\left( \pdv{J}{\mu} \right)_T
.\] 
This gives us
\begin{align*}
...
\item Below the transition temperature, the particles in the ground state do not contribute any energy, and hence the energy can be computed from the grand canonical partition function approximated using an integral
	\begin{align*}
	E &= -\left( \pdv{J}{T} \right) \\
	&= \pdv{T}\left[ \frac{(k_B T)^4}{(\hbar\omega)^3}\text{Li}_4(z) \right] \\
	&=4 k_B \left( \frac{k_B T}{\hbar\omega} \right)^3 \text{Li}_4(z) \\
	&=4k_B \left( \frac{k_B T}{\hbar\omega} \right)^3 \zeta(4)
\end{align*}
where we note that the chemical potential is $0$ under the transition temperature. The heat capacity is given by
	\[
	c_V = \pdv{E}{T} = 12 k_B^2 \frac{(k_B T)^2}{(\hbar\omega)^3}\zeta(4)
	.\qedhere\] 
	\end{parts}
\end{proof}
\begin{Problem}
...
.\] 
This is rather reminiscent of the previous problem; we define $\Omega(n)$ analogously. We note that $\Omega(n)$ solves a Stars and Bars problem. Essentially, we imagine arranging the $n$ books in a line and partition them into the $k$ boxes by placing $k-1$ dividers such that the books within a single group into a single box. We can arrange them by imagining the $n$ books and $k-1$ dividers as $n+k-1$ symbols, and we must choose $k-1$ to be dividers. Filling in the rest of the symbols with books yields our partitions. Thus, we have
\[
