...
\item Below the transition temperature, the number of particles in excited states remains at
	...
	.\] 
\item Below the transition temperature, the particles in the ground state do not contribute any energy, and hence the energy is just
	\[
	E = 
	.\] 
	\end{parts}
...
.\] 
In general, the density of modes per unit volume for a given wavenumber $k$ is $k^{d-1}\dd{k}$ (the prefactors all do not matter, as we can simply normalise it - only the scaling really matters).

Thus, using the Bose-Einstein distribution, we find that the average particle number and energy are given by (absorbing the normalisation of the partition function into a constant $A$)
\begin{align*}
	n &= A\int_0^\infty \frac{k^{d-1}}{e^{\beta\hbar ck}-1}\dd{k} \\
	  &= \frac{A}{(\beta\hbar c k)^d}\int_0^\infty \frac{(\beta\hbar c k)^{d-1}}{e^{\beta\hbar ck}-1}\dd{(\beta\hbar c k)} \\
	  &= \frac{A}{(\beta\hbar c k)^{d-1}}\Gamma(d)\text{Li}_d(1) \\
	u &= A\int_0^\infty \frac{\hbar ck^d}{e^{\beta\hbar c k}-1}\dd{k} \\
	  &= \frac{\hbar c A}{(\beta\hbar c)^{d+1}}\int_0^\infty \frac{(\beta\hbar c k)^d}{e^{\beta\hbar c k}-1}\dd{(\beta\hbar c k)} \\
	  &= \frac{\hbar c A}{(\beta\hbar c)^{d+1}}\Gamma(d+1)\text{Li}_{d+1}(1) \\
\end{align*}
We begin by computing the partition function. This is similar to the previous problem, but we can simply note that the density of states, which plays the function of $\Omega(n)$ in the previous problem, is simply proportional to the $d-1$-th power of $n$. Here, as we are taking the limits as the size of the box tends to infinity, the energy levels become continuous and we can replace the sums by an integral.
\end{proof}
