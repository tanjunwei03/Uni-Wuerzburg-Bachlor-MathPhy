...
\[
	\Delta F = A\frac{u}{4}\int \dd{x}[\psi_0^4 - \psi^4(x)]
.\] 
...
.\] 
Now all we need to do is substitute this back in to get
\begin{align*}
	\sigma=\frac{\Delta F}A &= \frac u4 \psi_0^4 \int_{-\infty}^\infty \left[1-\tanh^4\left(\frac{x-x_0}{\sqrt{2} \xi}  \right) \right]\dd{x}\\
	&= \frac{\xi u}{\sqrt{8}} \psi_0^4 \int_{-\infty}^\infty \left[1-\tanh^4\left(u\right) \right]\dd{u}\\
	&= \frac{\sqrt{8}}3\xi u\psi_0^4.\qedhere
\end{align*}
\end{proof}
\begin{Problem}
...
\begin{proof}
	...
	.\] 
	This corresponds to, in terms of the components, if $\psi = (\psi_1, \psi_2)^T$
	\[
	...
	\[\pdv{x} \begin{pmatrix}
		\psi_1 \\ \psi_2
	\end{pmatrix}=-im(x)\begin{pmatrix}
	0 & 1 \\ 1 & 0
\end{pmatrix}\begin{pmatrix}
\psi_1 \\ \psi_2
\end{pmatrix}=m(x)\sigma_2 \begin{pmatrix}
\psi_1 \\ \psi_2
\end{pmatrix}\]
This is a linear differential equation and can be solved using the matrix exponential
\[\begin{pmatrix}
	\psi_1(x) \\\psi_2(x)
\end{pmatrix}=\exp\left(\sigma_2 \int_0^x m(x)\dd{x}\right)\begin{pmatrix}
\psi_1(0) \\ \psi_2(0)
\end{pmatrix}\]
While this matrix exponential can be explicitly evaluated, we can proceed in a more clever manner. The zero energy state must be normalisable, and hence must decay at $\pm\infty$. Since the function $m(x)$ is even, we can write
\[\begin{pmatrix}
	\psi_1(x) \\\psi_2(x)
\end{pmatrix}=\exp\left(\sigma_2 \int_0^{|x|} m(x)\dd{x}\right)\begin{pmatrix}
	\psi_1(0) \\ \psi_2(0)
\end{pmatrix}\]
from which it is clear that the integral is always positive. Now, since $\sigma_2$ has eigenvalues $\pm 1$, we must choose the negative eigenvector $(1,-i)$. Thus, we find that the state is given by, up to a constant prefactor,
\[\begin{pmatrix}
	\psi_1(x) \\\psi_2(x)
\end{pmatrix}=\exp\left(\sigma_2 \int_0^{|x|} m(x)\dd{x}\right)\begin{pmatrix}
	1 \\ -i
\end{pmatrix}=\exp\left(-\int_0^{|x|} m(x)\dd{x}\right)\begin{pmatrix}
1 \\ -i
\end{pmatrix}.\qedhere\]
\end{proof}
