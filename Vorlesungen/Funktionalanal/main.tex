...
\begin{Definition}
	...
	\begin{parts}
	\item The polar $A^*$ of a subset $A\subseteq V$ is defined by
		\[
			A^*=\{\varphi\in V'| |\varphi(v)|\le 1\text{ for all }v\in A\} 
		.\] 
	\item The polar $B_*$ of a subset $B\subseteq V'$ 
		\[
			B_* = \{v\in V| |\varphi(v)|\le 1\text{ for all }\varphi\in B\} 
	.\] 
	\end{parts}
\end{Definition}
\begin{Theorem}
	Let $V$ be a topological vector space and $B\subseteq V'$. Then $B_*\subseteq V$ is absolutely convex and weakly closed.
\end{Theorem}
\begin{proof}
	For $z\in \mathbb{K}$ with $|z|\le 1$ and $v\in B_X$, then for $\varphi\in B$ we have $|\varphi(z v)| = |z| |\varphi(v)| \le 1$, which shows that $B_*$ is balanced.

	Then we choose $\lambda\in [0,1]$ and $v,w\in B_*$. Then for $\varphi\in B$ we have
	\[
	\varphi(\lambda v + (1-\lambda) w)\le \lambda |\varphi(v) + (1-\lambda)| \varphi(w)| \le 1
	.\] 
	This implies that $B_*$ is convex.

	To show that it is closed, we have
	\begin{align*}
		B_*&= \left\{ v\in V| |\varphi(v)| \le 1\text{ for all } \varphi\in B \right\} \\
