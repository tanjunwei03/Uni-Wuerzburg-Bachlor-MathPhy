...
\note{Theorem: There exists a one parameter group of automorphisms $\alpha_t$ on the quasi-local algebra $\mathcal{A}$ that extends the automorphism $\exp(t\delta)$ defined on the local algebra.
	...
	Proof: We will extend the group as follows. \note{Write this part in bullet points }First, we will begin with an interaction on a finite region. This will allow us to show isometry, whicdefined on the local algebra.h will then provide an extension to the quasi local algebra using well known results. Finally, we will extend the domain of $t$ from a neighbourhood of the origin to the entire real line.
