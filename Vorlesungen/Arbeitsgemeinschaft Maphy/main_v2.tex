...
\begin{Definition}
	A time evolution on a $C^*$ algebra $\mathcal{A}$ is a function $\alpha: \mathcal{A}\times \R\to \mathcal{A}$, that we denote as $\alpha_t$, such that each $\alpha_t$ is an *-automorphism. The pair $(\mathcal{A}, \alpha_t)$ is called a $C^*$ dynamical system.\note{ If it's not a star automorphism, you cannot prove that the time derivative is a derivation}
\end{Definition}
...
.\] 
In addition, the interactions will also often be \emph{finite range}, meaning that $\Phi(X)=0$ for sets larger than a certain diameter, say $|X| > N_\Phi$. If the interaction is finite range, then the sum of the local Hamiltonians induce a derivation, because
\[
...
\[
\|\delta^n(A)\|\le 2^{n+C_\Phi} \|A\|\prod_{k=1}^{n} ((k-1)(N_\Phi-1)+|\Lambda|)\|\Phi\|^n 
\] 
where $C_\Phi$ is a constant proportional to $N_\Phi$. By a simple computation, it follows that\note{ To prove that first estimate, first note that $\|[A,B]\| \le 2 \|A\| \|B\|$. Then, pick a point in $S_n$, of which there are $|S_n|$ many. Then the size of the $N_\Phi$ ball around this point is bounded, leading to the constant $C_\Phi$.}
\[
	\|\delta^n(A)\|\le \|A\|n! e^{\lambda(|\Lambda|+1-N_\Phi)}\left( \frac{2C_\Phi\|\Phi\|e^{\lambda(N_\Phi-1)}}{\lambda} \right)^n
.\] 
...
.\] 
We note that $A_s^2 = B_p^2 = \mathbbm{1}$. In particular, the eigenvalues of $A_s$ and $B_p$ are $\pm 1$. In addition, we have $[A_s,B_p]=0$ for all stars $s$ and plaquettes $p$. We can observe this as follows: Consider any star $s$ and plaquette $p$. Either they are disjoint, or they share exactly two spins. If they share two spins, we can pull one Pauli matrix through the other using the anticommutativity to 
