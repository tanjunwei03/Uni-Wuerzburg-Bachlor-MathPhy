...
\documentclass[prb,11pt, nofootinbib]{revtex4-2}
%fonts
...
\renewcommand{\note}[1]{{}}
\begin{document}
\title{Fractionalisation and Irreducible Representations of $C^*$ Algebras on Quantum Spin Systems}
	\author{Jun Wei Tan}
	...
	The proton was discovered at the turn of the 20th century by Ernest Rutherford. For a long time, it was thought to be a fundamental building block of matter. This would imply that charges are quantised in units of $1e$. However, we later discovered that protons are made of subatomic particles named \emph{quarks}, which each have a charge that is a fraction of $e$. 

However, these quarks are tightly bound together by the strong nuclear force, and hence require a high energy to separate. Is it then impossible to have particles with such a fractional charge? The answer is no. The paradigmatic model is the \emph{toric code}, which we will discuss today. However, a rigorous treatment of any many body system requires a significant amount of machinery, which we will now discuss.\note{

Today's talk will be about fractionalisation. What is fractionalisation? Generally, we choose quantum numbers to be integers. For example, the total charge of a system is an integer multiple of the number of electrons. Is it possible to obtain quantum numbers that are fractions?

The first thing you might think of might be physically splitting the particle up. (Draw circle for proton) For example, we know that the proton can be split into even smaller particles, which are two up quarks and one down quark. However, these require extremely high energies to break up. 


As it turns out, it is possible to have systems behave like the particles are fractional. Later, we will work with a system of spins, which are bosons. When swapping bosons, we obtain a sign of 1. We will see later that we get particles that act like fermions. As we know, fermions get a sign of -1 when we swap two of them. Thus, in some sense, fermions are half of bosons. Thus, we can think of them as fractional particles.

What kind of systems does fractionalisation happen in? We will work with lattice systems (draw lattice). These lattices could consist of a bunch of atoms. These atoms have orbitals on them, for example the s or p orbitals (draw). Electrons can live in these orbitals. In the case where the electrons cannot move and cannot swap between orbitals, they act like a single spin. These are known as spin systems, and will be the main focus of today's talk.

What concretely are these spin systems? We require a set of sites, which we will denote by $\Gamma$. A spin will live on each of these sites. The observables of spins are given by the $2\times 2$ hermitian matrices $H_2$. As we know previously, we require an algebra to describe the entire system. If we have any finite number of spins (draw a region on the lattice), we can construct an algebra using the tensor product. However, we also know that finite systems are boring.

For example, there are no phase transitions in finite systems. Mathematically, we know that all representations of a finite dimensional $C^*$ algebra are unitarily equivalent. Thus, we must be able to handle the thermodynamic limit. The initial thought that we might have is to use the algebraic tensor product
\[\bigotimes_{\Lambda\in \Gamma}\mathcal{A}_\Lambda\]
However, this is an incredibly large space. It includes operators that act across all of spacetime. These are generally not physical due to entropy considerations
}
...
\begin{Definition}[Quasi-Local Algebra]
	A quasi-local algebra is a $C^*$ algebra $\mathcal{A}$ and a net of $C^*$ algebras $\{\mathcal{A}_\alpha\}_{\alpha\in I}$ such that the net $I$ has an orthogonality relation $\perp$ and that satisfies the following conditions.
	\begin{enumerate}
		...
		\item There exists an automorphism $\sigma$ with $\sigma^2=\mathbbm{1}$, such that $\sigma(\mathcal{A}_\alpha)=\mathcal{A}_\alpha$ and
			\[
...
\begin{Definition}
	A time evolution on a $C^*$ algebra $\mathcal{A}$ is a function $\alpha: \mathcal{A}\times \R\to \mathcal{A}$, that we denote as $\alpha_t$ that is a strongly continuous 1 parameter group of *-automorphism. The pair $(\mathcal{A}, \alpha_t)$ is called a $C^*$ dynamical system.\note{ If it's not a star automorphism, you cannot prove that the time derivative is a derivation}
\end{Definition}
...
\begin{Example}
	Consider a $C^*$ dynamical system $(\mathcal{A}, \alpha_t)$. Then
	\[
	\delta(A) = \lim_{t \to 0}\left( \frac{\alpha_t(A) - A}{t} \right)  
\]
is a derivation, defined on the points where the limit exists.\note{ You show the product rule in the same way you show the product rule on an ordinary derivative: 
	\[H'(x) = \lim_{h\to 0}\frac1h\left[f(x+h)g(x+h)-f(x)g(x+h)+f(x)g(x+h)-f(x)g(x)\right]\]}
\end{Example}
...
\begin{Example}
	...
	\[
	\Phi(\{x,y\}) = \begin{cases}
		J\sum_{i=1}^3 \sigma_x^i \sigma_y^i & \langle xy\rangle\\
		0 & \text{otherwise.}
	\end{cases}
	\] 
\end{Example}
We recall again that we are working with \emph{spin systems}. This will become important here. In particular, it is important that the algebra on each site is finite. That means, if $\Lambda$ is bounded, then $\Phi(\Lambda)$ is finite dimensional. We call the algebra of observables that are supported in such a bounded set the local algebra $\mathcal{A}_\text{loc}$.

Thus, for each $\Lambda$, we can construct a \emph{bounded} Hamiltonian via
\[
	H(\Lambda) = \sum_{X\subseteq \Lambda}\Phi(X)
.\] 
In addition, the interactions will also often be \emph{finite range}, meaning that $\Phi(X)=0$ for sets larger than a certain diameter, say $\text{diam } X > D_\Phi$. This also implies that the number of elements in $X$ must be smaller than a certain value, say $|X|<N_\Phi$. If the interaction is finite range, then the sum of the local Hamiltonians induce a derivation on the local algebra, because
\[
	\delta^n(A) = i \lim_{\Lambda \to \infty}[H_\Lambda, A]
\]
only has finitely many terms where the support of $H_\Lambda$ intersects that of $A$.\note{ Probably comment something here about how this limit is in the sense of convergence of a net in the sense of the quasi-local algebra.}

Because of this, we can then show the time evolution for local systems:
\begin{figure}
	\begin{center}
...
\[
\|\delta^n(A)\|\le 2^{n+nN_\Phi} \|A\|\prod_{k=1}^{n} ((k-1)(N_\Phi-1)+|\Lambda|)\|\Phi\|^n 
\] 
By a simple computation, it follows that\note{ To prove that first estimate, first note that $\|[A,B]\| \le 2 \|A\| \|B\|$. Then, pick a point in $S_n$, of which there are $|S_n|$ many. Then the size of the $N_\Phi$ ball around this point is bounded, leading to the constant $C_\Phi$.}
\[
	\|\delta^n(A)\|\le \|A\|n! e^{\lambda(|\Lambda|+1-N_\Phi)}\left( \frac{2N_\Phi\|\Phi\|e^{\lambda(N_\Phi-1)}}{\lambda} \right)^n
\] 
where $\lambda$ is any positive real number. Thus, the exponential exists for sufficiently small $t$.\note{ We use the inequality $a^n \le n! \lambda^{-n}\exp(a\lambda)$ with $a=(n-1)(N_\Phi-1)+|\Lambda|$}
\end{proof}
Since the local observables form a dense subset of the quasi-local algebra, we can then extend the automorphism to the entire algebra by continuity. The technical elements remain to be proven --- the multiplicativity, continuity and so on, but these generally follow from the geometric series bound.
\note{Theorem: There exists a one parameter group of automorphisms $\alpha_t$ on the quasi-local algebra $\mathcal{A}$ that extends the automorphism $\exp(t\delta)$ defined on the local algebra.

	Proof: We will extend the group as follows. \note{Write this part in bullet points }First, we will begin with an interaction on a finite region. This will allow us to show isometry, which will then provide an extension to the quasi local algebra using well known results. Finally, we will extend the domain of $t$ from a neighbourhood of the origin to the entire real line.
	...
	.\] 
Note that $N(\Lambda)\xrightarrow{\Lambda\to\infty}\infty$.}
...
\end{Definition}
First, we note that this implies that the vector $\ket{\omega}$ in the GNS construction must be the eigenvector of eigenvalue 0:

\begin{Lemma}
	Let $\omega$ be a ground state of a $C^*$ dynamical system. In the GNS representation, $\ket{\omega}$ is the eigenvector of $H_\omega$ with zero eigenvalue.
\end{Lemma}
\begin{proof}
	The key ingredient is the invariance of $\omega$. First, we write the automorphism $\alpha_t$ as an inner automorphism, $\alpha_t(A) = U(t)^*AU(t)$. Then, invariance implies that
	\[\mel{\omega}{U(t)^*\pi_\omega(A)U(t)}{\omega}=\mel{\omega}{\pi_\omega(A)}{\omega}\]
	This, together with the group property, implies that $U(t)\ket{\omega}=e^{i\beta t}\ket{\omega}$ for some $\beta$ Differentiating and applying Stone's theorem yields the result 
	\[He^{i\beta t}\ket{\omega} =0\]
	which implies $H\ket{\omega}=0$ since $e^{i\beta t}$ is nonvanishing.
\end{proof}
...
\begin{Theorem}
	...
	\end{enumerate}
\end{Theorem}
To illustrate why this ostensibly opaque theorem should be true, let us return to the case of operators on a Hilbert space, where $\delta(A)=i[H,A]$. In addition, we assume that we are in the GNS representation, where $\omega$ is represented by a vector state $\ket{\omega}$. Since Substituting this into the second expression, we find
\begin{align*}
-i\omega(A^*\delta(A)) &= -i \mel{\omega}{A^*\delta(A)}{\omega}\\
&= \mel{\omega}{\left[A^*(HA - AH)\right]}{\omega}\\
&= \mel{\omega}{\left[A^*(HA - AE_0)\right]}{\omega}\\
&= \mel{\omega}{\left[A^*(H-E_0)A\right]}{\omega}\\
&\ge 0
\end{align*}
where $E_0=0$ is the minimal eigenvalue.
...
\end{center}
\end{figure}
The toric code, first proposed by Kitaev \cite{kitaev2003fault} in 1997, remains a paradigmatic model of topological physics. What is topological physics? Physics is topological when it is not local. For example, a magnetisation measurement is local, because magnetisation is a field that can vary from place to place. Because of this, magnetisation can be destroyed at high temperatures by thermal fluctuations, which act locally. However, a topological system is characterised by behaviour that explicitly cannot be expressed locally. Thus, they are highly robust, which is important for many applications.

 It is defined on a square lattice, with a spin degree of freedom on each \emph{edge}. We then define two operators, one defined on a ``star'' and one on a plaquette (see Fig. \ref{fig:toriccode}):
\[
	A_s = \bigotimes_{j\in s}\sigma_j^x,\qquad B_p = \bigotimes_{j\in p}\sigma_j^z
.\] 
We note that $A_s^2 = B_p^2 = \mathbbm{1}$. In particular, the eigenvalues of $A_s$ and $B_p$ are $\pm 1$. In addition, we have $[A_s,B_p]=0$ for all stars $s$ and plaquettes $p$. We can observe this as follows: Consider any star $s$ and plaquette $p$. Either they are disjoint, or they share exactly two spins. If they share two spins, we can pull one Pauli matrix through the other using the anticommutativity of two Pauli matrices.

The Hamiltonian is then given by
\[H = -\sum_s A_s - \sum_p B_p\]

Now we seek a ground state. Let us begin by showing that $A_s$ and $B_p$ are \emph{stabilisers}.
\begin{Lemma}
	Let $\mathcal{A}$ be a unital $C^*$ algebra,  $\omega$ a state on $\mathcal{A}$. Define $X\in \mathcal{A}$ such that $X=X^*$, $\omega(X)=1$ and $\mathbbm{1}-X$ is positive. Then for all $Y\in \mathcal{A}$ we have $\omega(XY)=\omega(YX)=\omega(Y)$.
\end{Lemma}
\begin{proof}
	Because $\mathbbm{1}-X$ is positive, we can take the square root $\mathbbm{1}-X$. Then, we have
	\begin{align*}
		|\omega( (\mathbbm{1}-X)Y)|^2 &= |\omega(\sqrt{\mathbbm{1}-X}\sqrt{\mathbbm{1}-X}Y)|^2  \\ 
		&\le \omega(\mathbbm{1}-X)\omega(Y^* (\mathbbm{1}-X)Y)=0 
	\end{align*}
	\note{The last equality follows from linearity, and thus $\omega(\mathbbm{1}-X)=0$. Write down the fact that $\omega( (\mathbbm{1}-X)Y)=0$ }where we used the Cauchy-Schwarz inequality\note{ $|\omega(A^*B)|^2 \le \omega(A^*A)\omega(B^*B)$} before finding that $\omega(\mathbbm{1}-X)=0$. Similarly, we can show that $\omega(Y(\mathbbm{1}-X))=0$. Thus, by the linearity of states, we find that $\omega(XY)=\omega(YX)=\omega(Y)$. 
\end{proof}
Now, we show the stabiliser property of $A_s$ and $B_p$:
\begin{Theorem}
...
\begin{proof}
	...
	.\] 
	We do the same for the last two terms with $B_p$. Thus, we find that the expression is positive, and $\omega$ is then a ground state.
\end{proof}
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\draw[ultra thick, blue] (0.5,1.5) pic[scale=1.5] {cross=3pt} -- (0.5,2.5) -- (3.5, 2.5) pic[scale=1.5] {cross=3pt};
		\foreach \x in {1,3}
...
\subfloat[][ ]{%
	\begin{tikzpicture}
		...
		\draw[ultra thick, red] (2, 4) pic[scale=1.5] {cross=3pt} -- ++(0,-2) -- ++(1,0) -- ++(0,-1) pic[scale=1.5] {cross=3pt};
	...
	\caption{Detection of defects using Wilson loops. \textbf{(a)} A magnetic defect is detected using a Wilson loop of star operators. \textbf{(b)} An electric defect is detected using a Wilson loop of plaquette operators.}
